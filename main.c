...
static void render_schedule(int n, char** dates, char*** cells) {
    ...
    for (int d=0; d<n; d++) {
        int dl = vis_len(dates[d]); if (dl+1 > cw[0]) cw[0] = dl+1;
        for (int s=0; s<6; s++) {
            int vl = cells[d][s] ? vis_len(cells[d][s]) : 1;
            if (vl+1 > cw[s+1]) cw[s+1] = vl+1;
        }
    }
    int full = 23;
    ...
    int pp[8]; pp[0] = 1;
    int acc = 0;
    for (int bi=0; bi<6; bi++) { acc += cw[bi]; pp[bi+1] = 1 + acc + 3*(bi+1); }
    pp[7] = full - 1;
